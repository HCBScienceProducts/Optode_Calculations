function out=refit3830batchcalibrationcertificate(in,modeltype,refittype)
% function out=refit3830batchcalibrationcertificate(in,modeltype,refittype)
%
...
% temp            - 1x5 matrix of temperatures (in °C)
% phase           - 7x5 matrix of phase readings (in °)
% p_cal           - unique value or 1x5 matrix of gas pressure (N2 & O2)
%                   used during calibration (in hPa)
% sat (optional)  - 7x1 matrix of mixing ratio values O2/N2, if empty:
...
% usage example:
% out=refit3830batchcalibrationcertificate(aadi3830_1219_b2408);
% out=refit3830batchcalibrationcertificate(aadi3830_1219_b2408,'Bittig');
% out=refit3830batchcalibrationcertificate(aadi3830_1219_b2408,[],'c');

%% check inputs
if ~isempty(setdiff({'temp';'phase';'p_cal';'airsaturated';'zerosolution';'serial_number'},fieldnames(in)));
    disp(['     Mandatory field(s) ' strjoin(setdiff({'temp';'phase';'p_cal';'airsaturated';'zerosolution';'serial_number'},fieldnames(in)),', ') ' are missing!'])
    return
end
if ~isfield(in,'sat'), in.sat=[0;1;2;5;10;20.9;30]; end
...
out.refittype=refittype;

%% start to reorganize input data
[n,m]=size(in.phase);
phase=in.phase;
if size(in.temp,2)==m && size(in.temp,1)==1
    temp=ones(n,1)*in.temp;
elseif size(in.temp,2)==1 && size(in.temp,1)==m % temperature flipped...
    temp=ones(n,1)*in.temp';
elseif ~isempty(setdiff(size(in.temp),[n m]))
    disp('Check size of temperature input! Processing might fail..')
end
p_cal=nanmean(in.p_cal);
out.p_calibrationgases=p_cal;
if size(in.sat,2)==1 && size(in.sat,1)==n
    sat=in.sat*ones(1,m);
elseif size(in.sat,2)==n && size(in.sat,1)==1 % saturation flipped...
    sat=in.sat'*ones(1,m);
elseif ~isempty(setdiff(size(in.sat),[n m]))
...
% and O2
pO2=p_cal.*sat/100; % / hPa

% convert equilibration partial pressure to oxygen concentration
% (=input to fitting routine only for original Uchida et al. 2008 model)
O2conc=O2ptoO2c(pO2,temp,0,0); % / umol L-1; freshwater

% convert to column vectors
temp=temp(:);phase=phase(:);pO2=pO2(:);O2conc=O2conc(:);

%% refit 7x5 foil calibration data according to specified modeltype 
% (batch data)
...
%%{
%% add visual check
note=char({['Aanderaa batch certificate refit type ' refittype],['batch no. ' num2str(batchno,'%.4u') ' -> 3830 SN ' num2str(in.serial_number,'%.4u')]});
