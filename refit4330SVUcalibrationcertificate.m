function out=refit4330SVUcalibrationcertificate(in,modeltype)
% function out=refit4330SVUcalibrationcertificate(in,modeltype)
%
% refit original Aanderaa multipoint calibration certificates to match,
% e.g., the GEOMAR modification of the Uchida et al. 2008 model after
% Bittig et al. 2017
%
% Note that the Aanderaa multipoint calibrations are performed as sensors
% vs. (reference) sensors. O2 reference data in the calibration sheets is
% from other optode sensors using the SVU model, not from individual
% Winkler samples. Thus, the O2 reference data implicitly include an
% assumption about optode O2-T-response (the Uchida et al. 2008 model).
% RMSE values are thus very small (and smaller than for a comparable number
% of independent Winkler samples) for both the Uchida et al. 2008 and
% Bittig et al. 2017 models.
%
% inputs: structure 'in' with fields
% temp            - 40x1 matrix of temperatures (in °C)
% phase           - 40x1 matrix of phase readings (in °)
% O2ref           - 40x1 matrix of O2 reference (in umol L-1)
% serial_number   - optode serial number
% batch_number (optional)         - number of foil batch (should terminate
%                                   with an 'M')
...
% usage example:
% out=refit4330SVUcalibrationcertificate(aadi4330_2182_b1206EM);
% out=refit4330SVUcalibrationcertificate(aadi4330_2182_b1206EM,'Bittig');

%% check inputs
if ~isempty(setdiff({'temp';'phase';'O2ref';'serial_number'},fieldnames(in)));
    disp(['     Mandatory field(s) ' strjoin(setdiff({'temp';'phase';'O2ref';'serial_number'},fieldnames(in)),', ') ' are missing!'])
    return
end
...
%% refit 40x1 foil calibration data according to specified modeltype 
...
%%{
%% add visual check
note=char({'Aanderaa multi-point certificate refit',['batch no. ' num2str(batchno,'%.4u') ' -> 4330 SN ' num2str(in.serial_number,'%.4u')]});
